...
# needed otherwise you cant call force_login (sync) in the function (async)
...
class TestWebsockets:
    ...
    async def test_rider_is_added_to_trip_groups_on_connect(self, settings):
        ...
        trip = await create_trip(pick_up_address="A", drop_off_address="B", rider=user)
    ...
    async def test_rider_is_alerted_on_trip_update(self, settings):
        ...
        channel_layer = get_channel_layer()
        await channel_layer.group_add(group=f"{trip.id}", channel="test_channel")
...
async def connect_and_create_trip(*, user, pick_up_address="A", drop_off_address="B"):
    communicator = await auth_connect(user)
